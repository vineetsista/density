...
from density.bench.synth import PERSONAS, SynthStats, generate
...
def test_all_three_personas_present(corpus, parsed) -> None:
    ...
    for persona in PERSONAS:
        ...
        # The exact serialized bytes of the prompt must recur verbatim, this
        # is the redundancy that dedup downstream is supposed to exploit.
        enc = json.dumps(prompt, ensure_ascii=False).encode("utf-8")
        assert blob.count(enc) >= 20
